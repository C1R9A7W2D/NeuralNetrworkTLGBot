...
namespace NeuralNetworkZodiac
{
    class AccordNet : BaseNetwork
    {
        ...
        {
            network = new ActivationNetwork(new SigmoidFunction(2.0),
                                          structure[0],
                                          structure.Skip(1).ToArray());
        ...
        public override int Train(Sample sample, double acceptableError, bool parallel)
        {
            ...
            do
            {
                error = teacher.Run(sample.input, sample.Output);
                ++iters;
            } while (error > acceptableError && iters < 1000);
